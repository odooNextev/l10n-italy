...
class TestAltriDatiGestionali(Common):
    ...
    def test_model_creation_with_text_ref(self):
        ...
        other_data = self.env["l10n_it_edi.move.line.other.data"].create(
            {
    ...
    def test_validation_name_too_long(self):
        """Test validation error when name exceeds 10 characters."""
        invoice = self.init_invoice(
            "out_invoice",
            amounts=[100],
            company=self.company,
        ...
        with self.assertRaises(ValidationError):
            self.env["l10n_it_edi.move.line.other.data"].create(
                {
                    "move_line_id": line.id,
                    "name": "TOOLONGNAME1",  # 12 characters
                    "text_ref": "Test",
                }
            )

    def test_validation_text_ref_too_long(self):
        """Test validation error when text_ref exceeds 60 characters."""
        invoice = self.init_invoice(
            "out_invoice",
            amounts=[100],
            company=self.company,
