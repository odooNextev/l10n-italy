...
l'anteprima XML ed il calcolo dei codici fiscali, la gestione facilitata
delle date per le fatture differite, una maggiore flessibilità nella
numerazione, miglioramenti nell'importazione e nell'esportazione delle
fatture XML.
...
5. Aggiunge di un wizard per calcolare i codici fiscali attingendo dai
...
code calculation, simplified date management for deferred invoices,
greater flexibility in numbering, and improvements in importing and
exporting XML invoices.
...
2. Invoice Number Regeneration (if not sent):
   ...
   -  If an invoice has not yet been sent to SDI (Exchange System), by
      bringing it back to draft and revalidating it, the module allows
...
5. Adds a wizard to calculate fiscal codes drawing from Italian
